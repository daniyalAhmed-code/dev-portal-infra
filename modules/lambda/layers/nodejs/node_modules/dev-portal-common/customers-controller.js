...
async function __createApiKey ({ identityId, userId, enabled }) {
  ...
  try {
    apiKey = await apigateway
      ...
      .promise()
      console.log("Here is apikey "+apiKey)
  } catch (error) {
...
const saveInvitePreLoginAccount = async ({
  userId,
  username,
  emailAddress,
    FirstName,
  ...
  console.log(KeyRotationEnabled)
  
  var params = {
    Description: "User CallbackAuth type", 
    Name: "vap/production/user-callback-auth/"+username, 
   };
   console.log(CallbackAuthType)
   if(CallbackAuthType == "apiKey")  {
    params['SecretString'] = JSON.stringify({ "apiKey": CallBackAuth });
  }
  else if (CallbackAuthType == "basicAuth"){
    params['SecretString'] = JSON.stringify({ "username": CallBackAuth.username,  "password": CallBackAuth.password});  
  }
  else if (CallbackAuthType == "privateCertificate"){
    params['SecretString'] = JSON.stringify({ "PrivateCertificate": CallBackAuth });
  } 
  let secretManagerRes = await secretsmanager.createSecret(params).promise()
  ...
  __savePreLoginAccount({
    ...
    MnoLocation:MnoLocation,
    Mno:Mno,
    CallBackAuthARN:secretManagerRes.ARN,
    CallBackUrl:CallBackUrl,
    Mfa:Mfa,
    ProfileImage:ProfileImage,
    KeyRotationEnabled:KeyRotationEnabled,
    CallbackAuthType:CallbackAuthType,
...
const createAccountInvite = async (
  { type,
    targetEmailAddress,
    targetPhoneNumber,
    targetFirstName,
  ...
  const updateUserAttribues = await cognitoIdp
  ...
  const preLoginAccount = await saveInvitePreLoginAccount({
    ...
    ApiKeyDuration: targetApiKeyDuration,
    MnoLocation:  targetMnoLocation,
    Mno: targetMno,
    CallBackAuth: targetCallBackAuth,
    CallBackUrl:  targetCallBackUrl,
    Mfa:  targetMfa,
    ProfileImage: targetProfileImage,
    KeyRotationEnabled: targetKeyRotationEnabled,
    inviterUserId,
    inviterEmailAddress,
    CallbackAuthType:type,
  })
  if (targetMfa){
    let params = {
      UserPoolId: getEnv("UserPoolId"), /* required */
      Username: targetUser.Username, /* required */
      SMSMfaSettings: {
        Enabled: true,
        PreferredMfa: true
      }
  ...
  console.log(preLoginAccount)
  console.log(params)
  let cognitoResp = await cognitoIdp.adminSetUserMFAPreference(params).promise()
  console.log("cognito resp below")
...
let getAccountDetails = async(userId) => {
  let cust = null
  let params = {
    TableName : getCustomersTableName(),
...
let updateAccountDetails = async (
  user_id,
  secretArn,
  data
) => { 
  
  let secretManagerParams = {
    SecretId: secretArn
  }
  ...
  if (data.CallBackAuth != "NONE")
  {  
    if(data.CallbackAuthType == "apiKey")  {
      secretManagerParams['SecretString'] = JSON.stringify({ "apiKey": data.CallBackAuth });
    }
    else if (data.CallbackAuthType == "basicAuth"){
      secretManagerParams['SecretString'] = JSON.stringify({ "username": data.CallBackAuth.username,  "password": data.CallBackAuth.password});  
    }
    else if (data.CallbackAuthType == "privateCertificate"){
      secretManagerParams['SecretString'] = JSON.stringify({ "PrivateCertificate": data.CallBackAuth });
    } 
    console.log(secretManagerParams)
    let secretManagerRes = await secretsmanager.putSecretValue(secretManagerParams).promise()
  }
  else 
    delete data.CallBackAuth
  
  if (data.Mfa){
    cognitoParams["SMSMfaSettings"] = {
      Enabled : true,
      PreferredMfa: true
    }
  }
  ...
  let cognitoResp = await cognitoIdp.adminSetUserMFAPreference(cognitoParams).promise()
...
let getSecretDetails = async(secretArn) => {
  ...
  try {
    let valRes = await secretsmanager.getSecretValue(params).promise();
    secret_string = valRes.SecretString
  }
  catch(error) {
    console.log(error)
  }
...
async function renewApiKey ( identityId, userId, enabled ) {
  console.log(`Creating API Key for identity ID [${identityId}]`)
  let updatedApiKey
  try {
...
async function deletePreviousApiKey ( apiKey) {
  let delete_apiKey
  let params = {apiKey:apiKey}
  try {
    delete_apiKey = await apigateway
      .deleteApiKey(params)
      .promise()
  } catch (error) {
    console.error('Failed to create API key:', error)
